...
async fn main() -> Result<()> {
    ...
    println!("║  Fetch Time:           {:>6} ms                                     ║", stats.fetch_time_ms);
    println!("╠═══════════════════════════════════════════════════════════════════════╣");
    ...
    let test_queries = ["roboto", "mono", "sans", "serif", "display", "code", "open", "inter"];
    ...
    for query in ["monospace", "script", "handwriting", "gothic", "vintage", "modern"] {
        let start = Instant::now();
    ...
    for query in ["noto", "arabic", "chinese", "japanese", "korean", "hebrew", "cyrillic"] {
        let (results, elapsed) = search.search_timed(query).await?;
